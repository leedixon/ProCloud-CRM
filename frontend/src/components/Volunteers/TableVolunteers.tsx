...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/volunteers/volunteersSlice';
...
const TableSampleVolunteers = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (volunteersNotify.showNotification) {
      notify(
        volunteersNotify.typeNotification,
        volunteersNotify.textNotification,
      );
    }
  ...
  const handleViewAction = (id: string) => {
    router.push(`/volunteers/volunteers-view/?id=${id}`);
  };
