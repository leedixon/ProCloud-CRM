...
const ListVolunteers = ({
  ...
  const hasUpdatePermission = hasPermission(currentUser, 'UPDATE_VOLUNTEERS');
            ...
            <CardBox hasTable key={item.id} className={'rounded'}>
              <div className={'flex items-center overflow-hidden'}>
                ...
                >
                  ...
                  <div className={'flex-1 px-3'}>
                    <p className={'text-xs text-gray-500'}>EventsAttended</p>
                    <p className={'line-clamp-2'}>
                      {dataFormatter
                        .eventsManyListFormatter(item.events_attended)
                        .join(', ')}
                    </p>
                  </div>
                </div>
